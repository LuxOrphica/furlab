...
> **ZoneState** — не хранимое поле. Это вычисляемое UI-состояние (`NoResult` / `HasResult` / `Exported` / `NeedsRegeneration`), определяемое на основе наличия актуального `LayoutRun` и совпадения `paramsSnapshot`. В базе данных не сохраняется.
...
| constraints | JSON | Ограничения размещения (повороты, допуски; `allowMirror`, `allowFlip` — по умолчанию false) |
| minCoverageRatio | float, NULL | Минимальный коэффициент покрытия фрагмента куском (ρ_min, 0..1); не путать с `seamAllowanceReserveMm` (геометрический отступ на шов в NormalizeRules) |
...
| description | string, NULL | нет | "Короб №2, верхний слой" |
