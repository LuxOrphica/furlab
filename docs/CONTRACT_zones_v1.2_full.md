...
type ZoneOriginType = "base" | "split" | "legacy";
...
### 9.2. Редактирование split-зон с holes

В MVP отверстия не редактируются как отдельные объекты.

Разрешено:

- редактировать общую границу между зоной-вырезом и зоной-остатком через инструменты shared boundary;
- синхронно обновлять `hole` у зоны-остатка и `outer` у зоны-выреза.

Запрещено:

- двигать `holes` как отдельный объект;
- удалять вершины `holes` напрямую;
- менять форму `holes` без изменения соответствующей cut-зоны;
- объединять holes вручную.
...
- свободное редактирование holes как отдельного объекта;
- multipolygon-зоны;
