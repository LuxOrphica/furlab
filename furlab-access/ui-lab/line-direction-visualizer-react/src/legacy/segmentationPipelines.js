export function runSegmentationPipeline(input) {
  ...
  if (activePipeline === "v3") {
    contourSeed = buildPrimaryMaskV3(w, h, size, sourceData, runGrabCutMask);
  ...
  if (activePipeline === "v3") {
    ...
    // Rescue before refine: if V3 base mask collapsed, switch to tonal fallback.
    const baseStats = computeMaskStats(baseMask, w, h);
    const collapsed =
      baseStats.count < Math.max(3800, Math.floor(size * 0.05)) ||
      baseStats.bboxW < Math.max(32, Math.floor(w * 0.38)) ||
      baseStats.bboxH < Math.max(32, Math.floor(h * 0.38));
    if (collapsed) {
      // Rescue: ищем мездру по тональному диапазону.
      // Исключаем пиксели у границы изображения (фон сканера/тень), верхний порог снижен до 210.
      const rescueHigh = Math.min(tonalHigh, 210);
      const borderMargin = Math.max(4, Math.round(Math.min(w, h) * 0.01));
      const tonalMask = new Uint8Array(size);
      for (let y = borderMargin; y < h - borderMargin; y++) {
        for (let x = borderMargin; x < w - borderMargin; x++) {
          const idx = y * w + x;
          const g = sampleGray(x, y);
          if (g >= tonalLow && g <= rescueHigh) tonalMask[idx] = 1;
        }
      }
      const visitedRescue = new Uint8Array(size);
      const tonalMain = extractLargestComponent(tonalMask, visitedRescue, w, h);
      if (tonalMain.length) {
        baseMask = new Uint8Array(size);
        for (let i = 0; i < tonalMain.length; i++) baseMask[tonalMain[i]] = 1;
        mode = `tonal-rescue-${activePipeline}`;
        qualityFlags.push("v3_base_rescue");
      }
    }
  } else {
  ...
  if (useMezdraRefine) {
    ...
    refineDebug = refined.debugInfo || null;
...
function solidifyPieceMask(ctx) {
  ...
  // Radius must seal fur-hair gaps on the pelt edge so flood fill can't enter.
  // ~1.5% of shorter dimension gives ~5px on typical scans; cap at 8 to stay fast.
  const closeRadius = Math.max(2, Math.min(8, Math.round(minDim * 0.015)));
...
function refineMezdraMask(ctx) {
  ...
  if (isV3 && sourceData && isV3EdgeAwareEnabled()) {
  ...
  if (ratio < minRatio || ratio > 1.03) {
    return {
      ...
      debugInfo: { stage: "post_refine_guard", ratio, minRatio, maxRatio: 1.03, bboxBefore: { w: st.bboxW, h: st.bboxH }, bboxAfter: { w: st2.bboxW, h: st2.bboxH } },
    };
  }
  ...
  return {
    ...
    debugInfo: {
      stage: "refine_ok",
      ratio,
      bboxBefore: { w: st.bboxW, h: st.bboxH },
      bboxAfter: { w: st2.bboxW, h: st2.bboxH },
    },
  };
}

function isV3EdgeAwareEnabled() {
  try {
    const g = typeof window !== "undefined" ? window : globalThis;
    if (g && typeof g.__ldvV3EdgeAware === "boolean") return !!g.__ldvV3EdgeAware;
  } catch (_) {}
  return true;
}

function isV3MaterialAwareEnabled() {
  try {
    const g = typeof window !== "undefined" ? window : globalThis;
    if (g && typeof g.__ldvV3MaterialAware === "boolean") return !!g.__ldvV3MaterialAware;
  } catch (_) {}
  return false;
}

function isV3GraphCutEnabled() {
  try {
    const g = typeof window !== "undefined" ? window : globalThis;
    if (g && typeof g.__ldvV3GraphCut === "boolean") return !!g.__ldvV3GraphCut;
  } catch (_) {}
  return false;
}
...
function refineMezdraMaskV3EdgeAware(ctx) {
  ...
  if (startedAt && (Date.now() - startedAt) >= hardTimeoutMs) {
    return { mask, refineApplied: false, fallbackUsed: true, timeoutHit: true, reason: "hard_timeout_before_v3_edge_aware", debugInfo: { stage: "v3_edge_aware_start_timeout" } };
  }
  ...
  if (!seedMain.length) return { mask, refineApplied: false, fallbackUsed: true, timeoutHit: false, reason: "v3_seed_empty", debugInfo: { stage: "v3_seed_empty" } };
  ...
  for (let iter = 0; iter < growIters; iter++) {
    ...
    if (startedAt && (Date.now() - startedAt) >= hardTimeoutMs) {
      return { mask, refineApplied: false, fallbackUsed: true, timeoutHit: true, reason: "hard_timeout_v3_constrained_grow", debugInfo: { stage: "v3_constrained_grow_timeout", iter } };
    }
  }
  ...
  const borderIdx = [];
  for (let i = 0; i < size; i++) {
    const v = strongSeed[i] && !inner[i] ? 1 : 0;
    borderBand[i] = v;
    if (v) borderIdx.push(i);
  }

  const gray = buildGrayFromRgba(w, h, sourceData);
  const coreStats = computeGrayStatsOnMask(gray, inner, w, h);
  // Тюнинг против захвата ворса (сверху/снизу/по бокам):
  // делаем trim более чувствительным к "волосатой" нестабильной текстуре.
  ...
  for (let pass = 0; pass < 3; pass++) {
    if (startedAt && (Date.now() - startedAt) >= Math.max(120, hardTimeoutMs - 90)) {
      return {
        mask,
        refineApplied: false,
        fallbackUsed: true,
        timeoutHit: false,
        reason: "v3_budget_guard_pretrim",
        debugInfo: { stage: "v3_budget_guard_pretrim", pass, removed },
      };
    }
    const toDrop = [];
    for (let bi = 0; bi < borderIdx.length; bi++) {
      if (startedAt && bi % 300 === 0 && (Date.now() - startedAt) >= Math.max(120, hardTimeoutMs - 60)) {
        return {
          mask,
          refineApplied: false,
          fallbackUsed: true,
          timeoutHit: false,
          reason: "v3_budget_guard_intrim",
          debugInfo: { stage: "v3_budget_guard_intrim", pass, bi, removed },
        };
      }
      const i = borderIdx[bi];
      if (!out[i]) continue;
      const x = i % w;
      const y = (i / w) | 0;
  ...
  if (!finalMain.length) return { mask, refineApplied: false, fallbackUsed: true, timeoutHit: false, reason: "v3_empty_after_trim", debugInfo: { stage: "v3_empty_after_trim", removed } };
  const finalMask = new Uint8Array(size);
  for (let i = 0; i < finalMain.length; i++) finalMask[finalMain[i]] = 1;
  // Убираем артефакты "ворс/фон", которые вылезают и касаются рамки кадра.
  ...
  const bboxWRatio = stAfter.bboxW / Math.max(1, stBefore.bboxW);
  const bboxHRatio = stAfter.bboxH / Math.max(1, stBefore.bboxH);
  const aspectBefore = stBefore.bboxW / Math.max(1, stBefore.bboxH);
  const aspectAfter = stAfter.bboxW / Math.max(1, stAfter.bboxH);
  const aspectShift = Math.max(aspectBefore, aspectAfter) / Math.max(1e-6, Math.min(aspectBefore, aspectAfter));
  ...
  if (ratio < 0.40 || ratio > 1.05) {
    return {
      ...
      fallbackUsed: true,
      timeoutHit: false,
      reason: "v3_area_ratio_guard",
      debugInfo: { stage: "v3_guard", guard: "area_ratio", ratio, bboxWRatio, bboxHRatio, aspectBefore, aspectAfter, aspectShift, removed },
    };
  }
  // Anti-collapse guard: refuse narrow-strip collapses like 277x479 -> 11x404.
  if (bboxWRatio < 0.45 || bboxHRatio < 0.45 || aspectShift > 3.2) {
    return {
      mask,
      refineApplied: false,
      fallbackUsed: true,
      timeoutHit: false,
      reason: "v3_bbox_collapse_guard",
      debugInfo: { stage: "v3_guard", guard: "bbox_collapse", ratio, bboxWRatio, bboxHRatio, aspectBefore, aspectAfter, aspectShift, removed },
    };
  }
  if (startedAt && (Date.now() - startedAt) >= hardTimeoutMs) {
    return { mask, refineApplied: false, fallbackUsed: true, timeoutHit: true, reason: "hard_timeout_after_v3_trim", debugInfo: { stage: "v3_after_trim_timeout", removed } };
  }
  return {
    mask: finalMask,
    refineApplied: true,
    ...
    debugInfo: {
      stage: "v3_edge_aware_ok",
      removed,
      ratio,
      bboxBefore: { w: stBefore.bboxW, h: stBefore.bboxH },
      bboxAfter: { w: stAfter.bboxW, h: stAfter.bboxH },
      bboxWRatio,
      bboxHRatio,
      aspectBefore,
      aspectAfter,
...
function buildPrimaryMaskV3(w, h, size, sourceData, runGrabCutMask) {
  const mask = new Uint8Array(size);
  const d = sourceData;
  if (!d) return mask;
  const profile = estimateMezdraProfile(w, h, d);
  const { r0, g0, b0, l0 } = profile;
  const useMaterialAware = isV3MaterialAwareEnabled();
  const useGraphCut = isV3GraphCutEnabled();
  const strongSeed = new Uint8Array(size);
  const candidate = new Uint8Array(size);
  const bgSeed = new Uint8Array(size);
  const minDim = Math.max(1, Math.min(w, h));
  const strictFrame = Math.max(10, Math.round(minDim * 0.08));
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const i = y * w + x;
      const p = i * 4;
      ...
      const nearFrame = edgeDist <= strictFrame;
      if (!useMaterialAware) {
      ...
      // Candidate band: permissive, only material-like colors/tones.
      const strictTone = lum >= Math.max(102, l0 - 18) && lum <= Math.min(216, l0 + 20);
      const strictColor = colorDist2 <= 1150;
      const strictSat = sat <= 52;
      const strictHit = strictTone && isWarm && strictColor && strictSat && !likelyWhiteFur && !likelyDarkBg;
      const relaxedHit = inTone && isWarm && lowSat && !likelyWhiteFur && !likelyDarkBg;
      if ((nearFrame ? strictHit : relaxedHit)) candidate[i] = 1;
      // Strong seed: strict subset for stable region growing origin.
      if (strictHit) {
        strongSeed[i] = 1;
      }
    }
  }
  const frame = Math.max(10, Math.round(Math.min(w, h) * 0.06));
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      if (x < frame || y < frame || x >= w - frame || y >= h - frame) bgSeed[y * w + x] = 1;
    }
  }
  // Exclude likely QR sticker zone only in material-aware branch.
  if (useMaterialAware) {
    const qrMask = detectLikelyQrZoneMask(w, h, d);
    if (qrMask) {
      for (let i = 0; i < size; i++) {
        if (!qrMask[i]) continue;
        candidate[i] = 0;
        strongSeed[i] = 0;
        bgSeed[i] = 1;
      }
  ...
  if (!useMaterialAware) return mask;
  if (useGraphCut && typeof runGrabCutMask === "function") {
    const gcMask = runGrabCutMask({
      sourceRgba: sourceData,
      w,
      h,
      fgSeed: strongSeed,
      bgSeed,
    ...
    if (gcMask && gcMask.length === size) {
      const gcMain = extractLargestComponentLocal(gcMask, w, h);
      const gcStats = computeMaskStatsLocal(gcMain, w, h);
      const candMainGuard = extractLargestComponentLocal(candidate, w, h);
      const candStatsGuard = computeMaskStatsLocal(candMainGuard, w, h);
      const gcAreaRatio = gcStats.count / Math.max(1, candStatsGuard.count);
      const gcBboxWRatio = gcStats.bboxW / Math.max(1, candStatsGuard.bboxW);
      const gcBboxHRatio = gcStats.bboxH / Math.max(1, candStatsGuard.bboxH);
      const gcLooksExpanded = gcAreaRatio > 1.08 || gcBboxWRatio > 1.05 || gcBboxHRatio > 1.05;
      if (
        gcStats.count > Math.max(2000, Math.floor(size * 0.01)) &&
        !gcLooksExpanded
      ) {
        return gcMain;
      }
  ...
  const grown = growMaterialRegionFromSeed(w, h, strongSeed, candidate);
  // Guard against seed-collapse: if grow result is too narrow, fallback to largest candidate blob.
  const candMain = extractLargestComponentLocal(candidate, w, h);
  if (!candMain.length) return grown;
  const grownStats = computeMaskStatsLocal(grown, w, h);
  const candStats = computeMaskStatsLocal(candMain, w, h);
  const areaRatio = grownStats.count / Math.max(1, candStats.count);
  const bwRatio = grownStats.bboxW / Math.max(1, candStats.bboxW);
  const bhRatio = grownStats.bboxH / Math.max(1, candStats.bboxH);
  if (areaRatio < 0.45 || bwRatio < 0.50 || bhRatio < 0.50) return candMain;
  return grown;
}

function detectLikelyQrZoneMask(w, h, rgba) {
  const size = w * h;
  if (!rgba || !size) return null;
  const whiteish = new Uint8Array(size);
  for (let i = 0; i < size; i++) {
    const p = i * 4;
    const r = rgba[p];
    const g = rgba[p + 1];
    ...
    const lum = 0.2126 * r + 0.7152 * g + 0.0722 * b;
    const sat = Math.max(r, g, b) - Math.min(r, g, b);
    if (lum >= 190 && sat <= 46) whiteish[i] = 1;
  }
  const comps = listComponentsLocal(whiteish, w, h);
  if (!comps.length) return null;
  const minArea = Math.max(420, Math.floor(size * 0.00025));
  const maxArea = Math.max(minArea + 1, Math.floor(size * 0.08));
  let best = null;
  for (let ci = 0; ci < comps.length; ci++) {
    const c = comps[ci];
    if (c.count < minArea || c.count > maxArea) continue;
    if (c.minX <= 1 || c.minY <= 1 || c.maxX >= w - 2 || c.maxY >= h - 2) continue;
    const bw = c.maxX - c.minX + 1;
    const bh = c.maxY - c.minY + 1;
    const aspect = bw / Math.max(1, bh);
    if (aspect < 0.6 || aspect > 1.9) continue;
    const fillRatio = c.count / Math.max(1, bw * bh);
    if (fillRatio < 0.5) continue;
    const cx = (c.minX + c.maxX) * 0.5;
    const cy = (c.minY + c.maxY) * 0.5;
    const marginX = w * 0.1;
    const marginY = h * 0.1;
    if (cx < marginX || cy < marginY || cx > w - marginX || cy > h - marginY) continue;
    const contrast = estimateRectContrastLocal(w, h, rgba, c.minX, c.minY, c.maxX, c.maxY);
  ...
  const x1 = Math.min(w - 1, best.maxX + pad);
  const y1 = Math.min(h - 1, best.maxY + pad);
  for (let y = y0; y <= y1; y++) {
...
function estimateRectContrastLocal(w, h, rgba, minX, minY, maxX, maxY) {
  let inSum = 0;
  let inN = 0;
  let outSum = 0;
  let outN = 0;
  const ring = 3;
  for (let y = Math.max(0, minY - ring); y <= Math.min(h - 1, maxY + ring); y++) {
...
function listComponentsLocal(mask, w, h) {
  const size = w * h;
  const visited = new Uint8Array(size);
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      out.push({ count, minX, minY, maxX, maxY });
    }
  }
  return out;
}

function growMaterialRegionFromSeed(w, h, strongSeed, candidate) {
  const size = w * h;
  const out = new Uint8Array(size);
  ...
  // Seed from central area first to avoid accidental fur corners.
  ...
  for (let y = y0; y < y1; y++) {
    for (let x = x0; x < x1; x++) {
      const i = y * w + x;
      if (!strongSeed[i] || out[i]) continue;
      out[i] = 1;
      qx[tail] = x;
      qy[tail] = y;
      tail++;
  ...
  // Fallback: if central seed is empty, use any seed pixel.
  if (tail === 0) {
    for (let i = 0; i < size; i++) {
      if (!strongSeed[i]) continue;
      const x = i % w;
  ...
  while (head < tail) {
    ...
    for (let k = 0; k < 4; k++) {
      ...
      if (nx < 0 || ny < 0 || nx >= w || ny >= h) continue;
      const ni = ny * w + nx;
      if (out[ni] || !candidate[ni]) continue;
      out[ni] = 1;
      qx[tail] = nx;
      qy[tail] = ny;
      tail++;
...
function extractLargestComponentLocal(mask, w, h) {
  const size = w * h;
  const visited = new Uint8Array(size);
  const out = new Uint8Array(size);
  let best = null;
  ...
  if (!best || !best.length) return out;
  for (let i = 0; i < best.length; i++) out[best[i]] = 1;
  return out;
}

function computeMaskStatsLocal(mask, w, h) {
  let count = 0;
  let minX = w;
  let minY = h;
  let maxX = -1;
  ...
  if (!count) return { count: 0, bboxW: 0, bboxH: 0 };
  return { count, bboxW: maxX - minX + 1, bboxH: maxY - minY + 1 };
}
