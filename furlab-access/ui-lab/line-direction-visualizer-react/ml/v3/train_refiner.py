...
class RefinerDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        teacher = cv2.imread(str(it.teacher), cv2.IMREAD_GRAYSCALE)
        gt = cv2.imread(str(it.gt), cv2.IMREAD_GRAYSCALE)
        if img is None or teacher is None or gt is None:
            raise RuntimeError(f"Failed to read triplet: {it.image} / {it.teacher} / {it.gt}")

        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img = cv2.resize(img, (self.img_size, self.img_size), interpolation=cv2.INTER_AREA)
        teacher = cv2.resize(teacher, (self.img_size, self.img_size), interpolation=cv2.INTER_NEAREST)
        gt = cv2.resize(gt, (self.img_size, self.img_size), interpolation=cv2.INTER_NEAREST)

        if self.augment:
            if random.random() < 0.5:
                img = np.fliplr(img).copy()
                teacher = np.fliplr(teacher).copy()
        ...
        x_rgb = torch.from_numpy(img.transpose(2, 0, 1)).float() / 255.0
        x_teacher = torch.from_numpy((teacher > 127).astype(np.float32)).unsqueeze(0)
        x = torch.cat([x_rgb, x_teacher], dim=0)
        y = torch.from_numpy((gt > 127).astype(np.float32)).unsqueeze(0)
        return x, y
...
def build_items(root: Path) -> List[Item]:
    images = root / "images"
    teachers = root / "masks_teacher"
    gts = root / "masks_gt"
    ...
    for f in sorted(images.iterdir()):
        ...
        t = teachers / f.name
        g = gts / f.name
        if t.exists() and g.exists():
            out.append(Item(f, t, g))
    return out
...
def train(args) -> None:
    ...
    items = build_items(ds_root)
    if len(items) < 2:
        raise RuntimeError("Need at least 2 image+teacher+gt triplets")

    train_items, val_items = split_items(items, args.val_ratio)
    print(f"triplets={len(items)} train={len(train_items)} val={len(val_items)}")

    train_ds = RefinerDataset(train_items, args.img_size, augment=True)
    val_ds = RefinerDataset(val_items, args.img_size, augment=False)
    train_loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, num_workers=0)
    val_loader = DataLoader(val_ds, batch_size=1, shuffle=False, num_workers=0)

    device = torch.device("cuda" if torch.cuda.is_available() and not args.cpu else "cpu")
    model = UNetRefiner(in_channels=4).to(device)
